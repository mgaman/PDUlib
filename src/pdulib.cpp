...
*/
int PDU::utf8_to_packed7bit(const char *utf8, char *pdu, int *septets, int udhsize)
{
  int r;
  int w;
  int len7bit;
...
int PDU::encodePDU(const char *recipient, const char *message, unsigned short csms, unsigned char numparts, unsigned char partnumber)
{
  ...
  */
  ...
  case ALPHABET_7BIT:
    ...
    if (udhsize != 0)
    {
      memcpy(&smsSubmit[smsOffset], udhbuffer, udhsize);
      smsOffset += udhsize;
    }
  ...
  case ALPHABET_16BIT:
    ...
    pduLengthPlaceHolder = smsOffset;
    smsSubmit[smsOffset++] = 1; // placeholder for length in octets
    // insert UDH if any
    if (udhsize != 0)
    {
      memcpy(&smsSubmit[smsOffset], udhbuffer, udhsize);
